...
from cajas.scripts import build_offline_review_packet as offline_review_cli
...
class BuildOfflineReviewPacketCliTests(unittest.TestCase):
    def test_cli(self):
        with TemporaryDirectory() as tmp:
            ...
            rc = d / "rc.json"
            fr.write_text(
                json.dumps({"final_status": "blocked", "blocked_actions": [], "permitted_next_actions": []}),
                encoding="utf-8",
            )
            ...
            om = d / "review.md"
            argv = [
                "build_offline_review_packet.py",
            ...
            with mock.patch.object(sys, "argv", argv):
                rc_code = offline_review_cli.main()
            self.assertEqual(rc_code, 0)
            self.assertTrue(oj.exists())
            self.assertTrue(om.exists())
