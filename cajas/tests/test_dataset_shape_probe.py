...
class DatasetShapeProbeTests(unittest.TestCase):
    ...
    def test_compatible_shape(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            cfg = self._write_cfg(tmp_dir, self._write_dataset(tmp_dir))
            report = run_dataset_h_shape_probe(config_path=str(cfg))
            self.assertTrue(report.compatible_shape)
            self.assertEqual(report.feature_count, 6)

    def test_empty_segment_incompatible(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            cfg = self._write_cfg(
                tmp_dir,
            ...
            report = run_dataset_h_shape_probe(config_path=str(cfg))
            self.assertFalse(report.compatible_shape)
            self.assertTrue(any("Segment probe failed for train" in b for b in report.blockers))

    def test_report_serialization(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            cfg = self._write_cfg(tmp_dir, self._write_dataset(tmp_dir))
            payload = run_dataset_h_shape_probe(config_path=str(cfg)).to_dict()
            self.assertIn("segments", payload)
            self.assertIn("qlib_dataset_api", payload)
