...
def test_label_schema_allowed_values_and_ranges() -> None:
    ...
    assert "human_label" in allowed
    assert "human_confidence" in allowed
    ...
    assert "up" in allowed["trend_direction"]
    assert "down" in allowed["trend_direction"]
    assert "sideways" in allowed["trend_direction"]
    assert "mixed" in allowed["trend_direction"]
    assert "unclear" in allowed["trend_direction"]
    ...
    assert "review_confidence" in allowed
    ...
    assert payload["legacy_allowed_values"] == {}
