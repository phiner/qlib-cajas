...
def detect_trend_segments_from_mask(mask: pd.Series, *, max_false_gap: int = 0) -> list[tuple[int, int]]:
    """Return contiguous true-mask segments as (start_idx, end_idx) inclusive pairs."""
...
def select_segment_representative_anchor(
    *,
    ...
    """Pick representative index preferring mid-segment and avoiding late bars."""
...
def _emit_trend_segment_candidates(
    ...
    *,
    ...
    for seg_no, (start, end) in enumerate(segments, start=1):
        ...
        raw_trigger_count = len(segment_indices)
        emitted_count = min(int(cfg["max_candidates_per_segment"]), max(1, raw_trigger_count))
        suppressed_count = max(0, raw_trigger_count - emitted_count)
        ...
        if suppressed_count > 0:
            reasons.append("same_segment_duplicate_suppressed")
        if late_segment_anchor:
            reasons.append("late_segment_anchor")
        if near_segment_low:
            reasons.append("near_segment_low")
        if near_segment_high:
            reasons.append("near_segment_high")
        if rebound_after_anchor:
            reasons.append("rebound_after_anchor")
        if weak_follow_through:
            reasons.append("weak_follow_through_after_anchor")
        if excluded_late_reversal_anchor:
            reasons.append("excluded_late_reversal_anchor")
        ...
        _add_candidate(
            rows,
            {
                ...
                "segment_duplicate_suppressed_count": int(suppressed_count),
