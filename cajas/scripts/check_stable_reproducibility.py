...
def main() -> int:
    p = argparse.ArgumentParser(description="Check stable reproducibility from stable fingerprints.")
    ...
    rep = build_stable_reproducibility_report(left=left, right=right)
    out = Path(args.out).expanduser().resolve()
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(rep, ensure_ascii=True, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"stable reproducibility: {out}")
