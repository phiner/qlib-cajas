...
## Critical Requirement — Do Not Select Trend-End Reversal Anchors

This requirement is especially important.

For `short_trend_down_candidate`:

```text
Do not select a candle near the end of a down segment if the next few candles rebound strongly.
```

For `short_trend_up_candidate`:

```text
Do not select a candle near the end of an up segment if the next few candles sell off strongly.
```

These cases should not enter preferred review candidates as trend-continuation examples.
...
Use local range / ATR-like proxy for rebound detection if ATR is not already available.

For downtrend candidates, mark candidate as bad representative if:

```text
segment_position_fraction > 0.75
OR anchor_index is within last 4 bars of segment
OR next 1–4 candles rebound strongly
```

For uptrend candidates, mirror the logic:

```text
segment_position_fraction > 0.75
OR anchor_index is within last 4 bars of segment
OR next 1–4 candles sell off strongly
```

### Required reason codes

When such anchors are detected, add reason codes:
...
means:
- do not emit this candle as a preferred `short_trend_down_candidate` / `short_trend_up_candidate`
- do not include it in review template/batch by default
- count it in candidate pack report as filtered/suppressed

If the implementation keeps it for audit, it must be marked:

```text
representative_anchor=false
late_segment_anchor=true
rebound_after_anchor=true
...
1. Downtrend late rebound exclusion:
   - synthetic down segment
   - candidate at or near final low
   - next candles rebound strongly
   - assert excluded from preferred candidates

2. Uptrend late selloff exclusion:
   - synthetic up segment
   - candidate near final high
   - next candles sell off strongly
   - assert excluded from preferred candidates

3. Mid-segment valid representative:
   - candidate near middle of trend segment
   - no immediate reversal
   - assert retained as preferred representative
